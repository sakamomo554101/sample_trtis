...
def setup_ui(ctx_param, input_size):
    # setup file picker
    uploaded_file = st.file_uploader("Choose a image file", type="jpg")
    if uploaded_file is not None:
        # read image
        img, orig_img = get_byte_data_from_img(uploaded_file, size=input_size)
        st.write("image size is " + str(orig_img.size))
        ...
        # draw face boxes
        imgs_with_box = draw_face_box_from_json([orig_img], input_size, result_map[corr_id])
...
def main():
    ...
    # setup ui
    setup_ui(ctx_param=ctx_param, input_size=(input_height, input_width))
